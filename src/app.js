...
function renderChipList(container, colors, emptyText) {
  ...
  colors.forEach((color) => {
    ...
    button.innerHTML = `<span></span><strong>${color.hex}</strong>`;
...
function updateCompare() {
  ...
  compareResult.innerHTML = `<div class="compare-swatches" style="--a:${a.hex};--b:${b.hex}"></div><strong>${hueDelta}deg hue delta</strong><span>${ratio.toFixed(2)}:1 contrast</span>`;
}
...
function makeSlider(layout) {
  ...
  svg.append(defs, bg, track, handle);
  svg._cf = { gradient, handle, center, radius };
...
function makeCore() {
  ...
  core.setAttribute('aria-label', 'Toggle poppy color picker');
  core.addEventListener('click', () => setExpanded(!state.expanded));
  return core;
}
...
function createPetal(color, layerIndex, index, totalPetals, radius, rotation, staggerDelay, zIndex) {
  ...
  petal.style.setProperty('--breeze-tilt', `${(0.55 + (index % 5) * 0.18 + layerIndex * 0.12).toFixed(2)}deg`);
  petal.style.setProperty('--breeze-x', `${(((index + layerIndex) % 2 === 0 ? 1 : -1) * (0.8 + layerIndex * 0.45)).toFixed(2)}px`);
  ...
  petal.style.transition = `transform ${state.duration}ms ${BLOOM_EASING} ${staggerDelay}ms, opacity ${state.duration}ms ${BLOOM_EASING} ${staggerDelay}ms, background 180ms ease, filter 180ms ease, box-shadow 180ms ease`;
  ...
  petal.addEventListener('mouseenter', () => {
    ...
    item.hovered = true;
    updatePetalTransform(item, true);
  });
  ...
  const item = { el: petal, color, x, y, angle, hovered: false };
  state.petals.push(item);
  updatePetalColor(item);
  updatePetalTransform(item, false);
}
...
function setExpanded(expanded) {
  ...
  updateExpandedClass();
  for (const item of state.petals) updatePetalTransform(item, false);
}
...
function updatePetalTransform(item, immediate) {
  const scale = item.hovered ? 1.08 : 1;
  let x = item.x;
  let y = item.y;
  if (state.expanded && state.mouse && !item.hovered) {
    const dx = x - state.mouse.x;
    const dy = y - state.mouse.y;
    const distance = Math.sqrt(dx * dx + dy * dy);
    if (distance < 92) {
      const push = (1 - distance / 92) * 10;
  ...
  if (immediate) {
    item.el.style.transitionDuration = '150ms,150ms,180ms,180ms,180ms';
    window.setTimeout(() => {
      item.el.style.transition = `transform ${state.duration}ms ${BLOOM_EASING}, opacity ${state.duration}ms ${BLOOM_EASING}, background 180ms ease, filter 180ms ease, box-shadow 180ms ease`;
    }, 170);
  }
  item.el.style.transform = state.expanded
    ? `translate(${x}px, ${y}px) rotate(${item.angle + 90}deg) scale(${scale})`
    : `translate(0, 0) rotate(${item.angle + 90}deg) scale(0)`;
...
function updateSlider(output = colorOutput()) {
  ...
  const { gradient, handle, center, radius } = slider._cf;
...
function queueInteractiveUpdate(event) {
  ...
  state.raf = requestAnimationFrame(() => {
    for (const item of state.petals) updatePetalTransform(item, true);
    state.raf = 0;
  });
}
